...
// build/dev/javascript/fluffy/js/flustre.mjs
...
// build/dev/javascript/fluffy/fluffy.mjs
var FILEPATH = "src/fluffy.gleam";
var NotLoaded = class extends CustomType {
};
...
var LoadedUser = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var DataLoaded = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var FetchUserData = class extends CustomType {
};
var UserDataLoaded = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var User = class extends CustomType {
  constructor(id2, first_name, last_name, email, gender, phone, image) {
    super();
    this.id = id2;
...
function ping2() {
  return from(
    (dispatch) => {
      let _pipe = ping();
      let _pipe$1 = new DataLoaded(_pipe);
  ...
  return from(
    (dispatch) => {
      ...
      let _pipe$1 = map_promise(
        _pipe,
        (var0) => {
          return new UserDataLoaded(var0);
        }
      );
...
function users_decoder() {
  return list2(user_decoder());
}
function decoded_user(res) {
  return run(res, users_decoder());
}
function update2(_, msg) {
  if (msg instanceof Ping) {
    return [new Loading(), ping2()];
  } else if (msg instanceof DataLoaded) {
    let data = msg[0];
    return [new Loaded(data), none()];
  } else if (msg instanceof FetchUserData) {
    return [new Loading(), get_data()];
  } else {
    let data = msg[0];
    let decoded_users = decoded_user(data);
    if (decoded_users instanceof Ok) {
      let users = decoded_users[0];
      return [new LoadedUser(users), none()];
    } else {
      return [new NotLoaded(), none()];
    }
  }
...
function view_user(user) {
  return p(
    toList([]),
    toList([
      text2(user.first_name + " " + user.last_name + " " + user.email)
    ])
  );
}
function view_user_list(user_list) {
  return div(
    toList([id("user-list")]),
    (() => {
      let _pipe = user_list;
      return map(_pipe, (user) => {
        ...
        if (model instanceof NotLoaded) {
          ...
          return div(
            ...
            toList([
              ...
              button(
                toList([on_click(new Ping())]),
                toList([text2("Ping Again")])
              ),
              button(
                toList([on_click(new FetchUserData())]),
                toList([text2("FetchUserData Again")])
              )
            ])
          );
        } else {
